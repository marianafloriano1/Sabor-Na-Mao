...
type RootStackParamList = {
  home: undefined;
  paoq_lactose: { categoria?: string };
  cookie_lactose: { categoria?: string };
  brigadeiro_lactose: { categoria?: string };
  churros_lactose: { categoria?: string };
  palitos_diabetes: { categoria?: string };
  cheesecake_diabetes: { categoria?: string };
  quibe_diabetes: { categoria?: string };
  salmao_diabetes: { categoria?: string };
  sanduiche_gluten: { categoria?: string };
  pizza_gluten: { categoria?: string };
  pastel_gluten: { categoria?: string };
  brownie_gluten: { categoria?: string };
};

type ComidasInfantisRouteProp = RouteProp<RootStackParamList, "home">;

export default function ComidasInfantis() {
  const nav = useNavigation<NavigationProp<RootStackParamList>>();
  const route = useRoute<ComidasInfantisRouteProp>();
  ...
  // Restaura aba ativa ao voltar da receita
  useEffect(() => {
    const categoria = route.params?.categoria;
    if (categoria === "intolerancia") toggleCategory("intolerancia");
    else if (categoria === "gluten") toggleCategory("gluten");
    else if (categoria === "diabete") toggleCategory("diabete");
    else toggleCategory("intolerancia"); // padrão
  }, [route.params]);
